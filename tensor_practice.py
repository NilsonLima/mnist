...
def PReLU(feature):
	...
	a = tf.get_variable("a", shape=[1], initializer=tf.constant_initializer(0.25))

	#return PReLU activation
	return tf.nn.relu(feature) + tf.matmul(a, tf.minimum(tf.constant(0), feature))
		...
		with tf.Session( ) as session:
			...
			for j in range(FLAGS.epochs):
				...
				for i in range(num_batches):
					...
					session.run(train_step, feed_dict={x: batch[0], y_: batch[1], keep_prob: 1.0})
				...
				save_path = saver.save(session, FLAGS.ckpt_dir + '/mnist_model')
